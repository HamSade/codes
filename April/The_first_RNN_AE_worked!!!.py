...
def rnn_layer(x, previous_state, w, b):
     ...
     b_i=tf.reshape( tf.tile (b[0,:],[batch_size]), [batch_size, -1]) 
     b_o=tf.reshape( tf.tile (b[1,:],[batch_size]), [batch_size, -1])
     b_f=tf.reshape( tf.tile (b[2,:],[batch_size]), [batch_size, -1])
     b_g=tf.reshape( tf.tile (b[3,:],[batch_size]), [batch_size, -1])
...
state_enc_1 = tf.zeros([2, batch_size, rnn_width])
