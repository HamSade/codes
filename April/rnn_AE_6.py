...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr  8 16:03:03 2017
...
class rnn_block():
    ...
    def __init__(self, num_units):
        dd = tf.contrib.rnn.BasicLSTMCell(num_units,state_is_tuple=True)
        self.rnn = tf.contrib.rnn.MultiRNNCell([dd]*2, state_is_tuple=True)
        self.my_scope = None

    def __call__(self, inp, previous_state):
        ...
        output, state= self.rnn(inp, previous_state)
...
#with tf.variable_scope('enc', reuse=True) as enc:
#    enc.reuse_variables
...
#        enc_full = tf.reshape(enc_full, [-1,1, full_width, 1] )
...
init_enc = AE.rnn_enc.rnn.zero_state(tf.shape(X)[0], tf.float32)
init_dec = AE.rnn_dec.rnn.zero_state(tf.shape(X)[0], tf.float32)
