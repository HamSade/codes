...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 19 09:41:10 2017

@author: hsadeghi


This is a binary version with bits at the ouput where each bit in connected to inout and its immediate bit before
...
from binary import bitwise_layer_2
...
for i in range(1, num_output_bits):
    wb.append(tf.Variable(tf.random_normal([2 , 1], stddev=std_init_W ) ) )


weights['binary'] = wb; #tf.Variable(wb);
biases['binary']= tf.Variable(tf.random_normal( [num_output_bits+1], stddev=std_init_bias));
...
#    print('iteration', i)
    binary_rep = bitwise_layer_2( tf.reshape(y_pred_final[: , i], [-1 , 1]),  num_output_bits, weights['binary'], biases['binary'], tf, mode)
