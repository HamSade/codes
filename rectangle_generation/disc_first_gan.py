...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 18 12:43:40 2017
...
learning_rate_init = 0.02   #DCGAN = 0.0002
lr_dec_fac = 1.
noise_std_init = [0.]
noise_dec_fac = 1.
...
#disc_cost = tf.reduce_mean( tf.squared_difference(d_real, 1.)) + tf.reduce_mean( tf.squared_difference(d_fake, 0.))
#gen_cost = tf.reduce_mean(tf.squared_difference(d_fake, 1.))
...
# Wasserstein
gen_cost = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = d_fake, labels = tf.ones_like(d_real)))
disc_cost = 1/2* ( tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = d_real, labels = tf.ones_like(d_real))) \
...
#disc_cost_mse = disc_cost_real_mse + disc_cost_fake_mse
#gen_cost_mse = tf.reduce_mean(tf.squared_difference(d_fake, 1.)) 


## L1
#disc_cost_real_l1 = tf.reduce_mean( tf.abs(d_real- 1.))
#disc_cost_fake_l1 = tf.reduce_mean( tf.abs(d_fake))
#disc_cost_l1 = disc_cost_real_l1 + disc_cost_fake_l1
#gen_cost_l1 = tf.reduce_mean(tf.abs(d_fake - 1.)) 
...
#disc_cost = disc_cost_l1
#gen_cost = gen_cost_l1
...
print("Disc optimization Finished!")


#%%######################################################
##%%%%%%%        Training both afterwards!! ;)    ############
#######################################################
...
print("Joint optimization Finished!")
...
save_path = "/home/hsadeghi/Dropbox/september/rectangle_generation/disc_first_output.mat"
