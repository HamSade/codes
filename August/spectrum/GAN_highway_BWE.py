...
layer_widths = [input_dim + 2 * overlap] * 50
cost_lambda = 0.
comp_ratio_disc = 2 ** 7
...
noise_h_highway = tf.random_normal(shape = highway_output_h.get_shape().as_list(), stddev = noise_std)

with tf.variable_scope('disc'):
#    d_real = discriminator(X_h, comp_ratio_disc)
    d_real = discriminator( tf.multiply( X_h, noise_h_real) , comp_ratio_disc)
#    d_real = discriminator(x_real, comp_ratio_disc)
with tf.variable_scope('disc', reuse=True):
#    d_fake = discriminator(highway_output_h, comp_ratio_disc)
    d_fake = discriminator( tf.multiply( highway_output_h, noise_h_highway) , comp_ratio_disc)
...
#highway_adv_cost =  - tf.reduce_mean(tf.log(d_fake))
...
#highway_l1_cost = tf.reduce_mean(tf.abs(X_h - highway_output_h))
highway_l2_cost = tf.reduce_mean(tf.squared_difference(X_h, highway_output_h))
highway_cost = highway_adv_cost + cost_lambda * highway_l2_cost
...
#                
            ...
            with tf.device('/gpu:1'):
                _, highway_l2_cost_, highway_adv_cost_ = sess.run([highway_opt, highway_l2_cost,highway_adv_cost],
                                       feed_dict={X: batch_xs,
                                                        learning_rate: learning_rate_init,
                                                        noise_std : noise_std_init})
#####             Display logs per epoch step
                if i % display_step == 0:
                    print("epoch:", '%02d' % (epoch + 1),
                            "File:", '%02d' % (file_idx),
                          "iteration:", '%04d' % (i + 1),
                          "Discriminator_cost =", "{:.9f}".format( (10**4) * disc_cost_),#   "highway_cost =", "{:.9f}".format( (10**4) * highway_cost_),
                          "highway_l2_cost =", "{:.9f}".format( (10**4) * highway_l2_cost_ ),
                          "highway_adv_cost", "{:.9f}".format( (10**4) * highway_adv_cost_) )
...
save_path = "/home/hsadeghi/Dropbox/august/GAN_highway_output.mat"
