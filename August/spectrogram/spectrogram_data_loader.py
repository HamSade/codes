...
#    print('Sxx.shape', Sxx.shape)
    ...
    # Concatenating zeros
    Sxx_l =  np.concatenate((Sxx_l, np.zeros(shape=[Sxx_l.shape[0],1])), axis = -1)
    Sxx_h =  np.concatenate((Sxx_h, np.zeros(shape=[Sxx_h.shape[0],1])), axis = -1)
...
#%%
x = data_loader(1, 512)
Sxx_l, Sxx_h, maxi = data_parser(x, 512, 128)
#
