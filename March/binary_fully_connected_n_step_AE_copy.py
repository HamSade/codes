...
def encoder(x):
    ...
    layer={};
    layer[str(1)] = tf.nn.tanh(tf.add(tf.matmul(x, weights['encoder_h'+str(1)]),
                                   biases['encoder_b'+str(1)]))
    for i in range(2, n_hid+1):
        layer[str(i)] = tf.nn.tanh(tf.add(tf.matmul(layer[str(i-1)], weights['encoder_h'+str(i)]),
                                   biases['encoder_b'+str(i)]))
    #  Quantizers    
    layer_middle = tf.nn.tanh( tf.add( tf.matmul(layer[str(n_hid)], weights['middle']),
                                   biases['middle']))    
    return layer_middle
...
def decoder(x):
    ...
    for i in range(2,n_hid+1):
        layer[str(i)] = tf.nn.tanh(tf.add(tf.matmul(layer[str(i-1)], weights['decoder_h'+str(i)]),
                                   biases['decoder_b'+str(i)]))
        
    layer_last=tf.nn.tanh(tf.add(tf.matmul(layer[str(n_hid)], weights['last']),
                                   biases['last']))
    return layer_last
