...
#    layer[str(1)]=BatchNormalization(x, weights['encoder_h'+str(1)], hid_size[0])
    
    layer[str(1)]=tf.matmul( layer[str(1)] , weights['encoder_h'+str(1)]);
...
#        layer[str(i)] = tf.nn.tanh(tf.add(tf.matmul(layer[str(i-1)], weights['encoder_h'+str(i)]), biases['encoder_b'+str(i)]))
        layer[str(i)] = BatchNormalization(layer[str(i-1)], weights['encoder_h'+str(i)],hid_size[i-1]);

#        layer[str(i)]=tf.matmul( layer[str(i)] , weights['encoder_h'+str(i)]);
        layer[str(i)]= tf.nn.tanh(layer[str(i)]);
    
    
#    layer_middle=tf.nn.tanh(tf.add(tf.matmul(layer[str(n_hid)],weights['middle']), biases['middle']));
#    layer_middle=BatchNormalization(layer[str(n_hid)], weights['middle'], n_hidden_binary)
    layer_middle=tf.matmul( layer_middle , weights['encoder_h'+str(1)]);
...
#                                   biases['decoder_b1']))
    layer[str(1)] = BatchNormalization(x, weights['decoder_h1'],hid_size[-1]);
    layer[str(1)]=tf.nn.tanh(layer[str(1)]);
    
    for i in range(2,n_hid+1):
#        layer[str(i)] = tf.nn.tanh(tf.add(tf.matmul(layer[str(i-1)], weights['decoder_h'+str(i)]), biases['decoder_b'+str(i)]))
        layer[str(i)] = BatchNormalization(layer[str(i-1)], weights['decoder_h'+str(i)],hid_size[-i])
        layer[str(i)]= tf.nn.tanh(layer[str(i)])
        
        
#    layer_last=tf.nn.tanh(tf.add(tf.matmul(layer[str(n_hid)], weights['last']), biases['last']))
    layer_last=BatchNormalization(layer[str(n_hid)], weights['last'], input_dim)
