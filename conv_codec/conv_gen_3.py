...
@author: hsadeghi
Refinement info is fully connected
...
def mux(x_dec, ps_q, input_dim):
    ...
    w_mux = tf.get_variable('w_mux', [total_code_length, total_code_length],
                            initializer= tf.random_normal_initializer(stddev=0.01))
    x_ = full_layer(x, w_mux, 'g_mux') 
    
    w_mux_2 = tf.get_variable('w_mux_2', [total_code_length, input_dim],
                              initializer= tf.random_normal_initializer(stddev=0.01))
    x_ = full_layer(x_, w_mux_2, 'g_mux_2')
    
    w_mux_3 = tf.get_variable('w_mux_3', [input_dim, input_dim],
                              initializer= tf.random_normal_initializer(stddev=0.01))
    x_ = full_layer(x_, w_mux_3, 'g_mux_3')
...
def side_net(hs, mode ):
    ...
    for i in range(len(hs)):
        with tf.variable_scope('g_side_' + str(i) ):
            w_side = tf.get_variable('w_side', [ input_dim / (2**i) , code_lengths[i] ],
                                                        initializer= tf.random_normal_initializer(stddev=0.01))
            h_temp = full_layer(hs[i], w_side, 'g_side') 
