...
#%%
def data_loader(file_ind, input_dim, mdct_indicator=False): 
...
#    hpf_l [:, int(length/2) + nc: length]    = hpf[ :, int(length/2): length - nc ]
    ...
    highpass = np.fft.ifft(hpf);
    
    #make sure its real
    lowpass.astype(np.float64)
    highpass.astype(np.float64)

    low_down = ss.decimate(lowpass, 2)
    # zero insertion
    low_down = np.insert(low_down, slice(0,None),0., axis=-1)
    b, a = ss.butter(20, wn, btype='low', analog=False)
    low_recon = ss.lfilter(b, a, low_down)    
    return low_recon, highpass, lowpass
...
def data_parser(data, input_dim, batch_size, overlap=0, apply_mask=False):    
    ...
    # 2-band approach
    low_recon, highpass, lowpass = band_split(parsed_data)
    
    return low_recon, highpass, lowpass

#%%

#data= np.random.normal(size = [128,512])
#y = band_split(data) #, 512, 128)
#print(y[0].shape)
#print(y[1].shape)
#print(y[2].shape)
#%%
...
#%% check if highpass is real not complex
...
#h_i  = np.imag(h)
#l_i = np.imag(l)
#print('l_i_max', np.max(np.abs(l_i)))
#print('h_i_max', np.max(np.abs(h_i)))
#%%
