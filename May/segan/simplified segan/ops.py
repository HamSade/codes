...
def downconv(x, output_dim, kwidth=5, pool=2, init=None, uniform=False,
    ...
    x2d = tf.expand_dims(x, 2)  # =tf.expand_dims(x, -1)
...
def conv1d(x, kwidth=5, num_kernels=1, init=None, uniform=False, bias_init=None,padding='SAME'):
    ...
    with tf.variable_scope(name):
        ...
        # INPUT SHAPE WAS [batch_size, in_width, in_channels]
        
        # filter shape: [kwidth, in_channels, num_kernels]
        # For example for kwidth=31 and in_channels=1, num_kernels=1:
#            W= [ [w_0], [w_1], ..., [w_30] ]
...
def time_to_batch(value, dilation, name=None):
    with tf.name_scope('time_to_batch'):
        ...
        pad_elements = dilation - 1 - (shape[1] + dilation - 1) % dilation
         # Uses Defaul of zero pading (CONSTANT) 
         # Add pad_elements zeros to the end
...
def atrous_conv1d(value, dilation, kwidth=3, num_kernels=1,
    ...
    with tf.variable_scope(name):
        ...
        padding = [[0, 0], [ (kwidth/2) * dilation, (kwidth/2) * dilation],
                  [0, 0]]  #No pading for batches or channels, just for in_width (input_width)
...
def residual_block(input_, dilation, kwidth, num_kernels=1,
    ...
    #Exactly implements WAVENET!!!!!!!!!!!!!
    
    with tf.variable_scope(name):
        
        
        # Gated convolution! Just like Wavenet! WOW!  :o
        ...
        #make res connection WOW again! This is exactly the 1x1 block in Wavenet!
...
def deconv(x, output_shape, kwidth=5, dilation=2, init=None, uniform=False,
    ...
    x2d = tf.expand_dims(x, 2)  # [m,n] ==> [m,n,1]
    
    o2d = output_shape[:2] + [1] + [output_shape[-1]]  #[m,n,k] ==> [m,n,1,k]
