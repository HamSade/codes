...
print('lambda_hhl', lambda_hhl)
print('lambda_hhh', lambda_hhh)
print('comp_l', comp_l)
print('comp_hl', comp_hl)
print('comp_hhl', comp_hhl)
print('comp_hhh', comp_hhh)
...
X = tf.placeholder("float", [4, batch_size, input_dim + overlap * 2])
...
with tf. variable_scope('highhighlow'):
    y_pred_hhl = auto_encoder(X_hhl, mode,comp_hhl)
with tf. variable_scope('highhighhigh'):
    y_pred_hhh = auto_encoder(X_hhh, mode,comp_hhh)
#if emphasis:
...
cost_hhl = tf.reduce_mean( tf.pow( tf.add(y_true_hhl, -y_pred_hhl) , 2))
cost_hhh = tf.reduce_mean( tf.pow( tf.add(y_true_hhh, -y_pred_hhh) , 2))

cost = tf.add( cost_l, tf.add( lambda_hl * cost_hl ,
                              tf.add( lambda_hhl * cost_hhl, lambda_hhh * cost_hhh) ))

if emphasis:
    y_true_hhh = de_emph(y_true_hhh)
    y_pred_hhh = de_emph(y_pred_hhh)
...
#            _, c = sess.run([optimizer, cost], feed_dict={X: batch_xs, mode:0.0})
            _, c , cl, chl, chhl, chhh = sess.run([optimizer, cost, cost_l, cost_hl,
                                                   cost_hhl, cost_hhh],
...
#                      "cost =", "{:.9f}".format( (10**4) *c))
                print("epoch:", '%02d' % (epoch+1),
                      ...
                      "cost_hhl=", "{:.9f}".format( (10**4) *chhl),
                      "cost_hhh=", "{:.9f}".format( (10**4) *chhh)) 
...
for i in range(avg_num):
    ...
    y_true_l_, y_true_hl_, y_true_hhl_, y_true_hhh_,\
    y_pred_l_, y_pred_hl_, y_pred_hhl_, y_pred_hhh_,\
    test_error_ = sess.run([y_true_l, y_true_hl, y_true_hhl, y_true_hhh,\
                                      y_pred_l, y_pred_hl, y_pred_hhl, y_pred_hhh, cost],\
                                        feed_dict={X: sampled_x, mode:1.0})

    y_true_test_ = band_merge(y_true_l_, band_merge(y_true_hl_, band_merge(y_true_hhl_, y_true_hhh_)))
    y_pred_test_ = band_merge(y_pred_l_, band_merge(y_pred_hl_, band_merge(y_pred_hhl_, y_pred_hhh_)))
