...
class Encoder:
    def __init__(self, depths=encoder_depths):
        ...
        print('Encoder depths', self.depths)
        self.reuse = False
...
#        print('output size', outputs.get_shape().as_list())
        ...
        with tf.name_scope('coder' + name), tf.variable_scope('coder', reuse=self.reuse):
            ...
            for i in range(1,len(self.depths)):
                with tf.variable_scope('coder_conv_'+str(i)):                    
                    ### HIGHWAY METHOD
                    if i==1:
...
#                outputs = tf.nn.relu(outputs) #to make sure quantizer inputs are in [-1, 1]
        self.reuse = True
...
#            inputs = tf.expand_dims(inputs, axis=-1)
        with tf.variable_scope('coder', reuse=self.reuse):
            ...
            noise_shape = outputs.get_shape().as_list()
            noise_shape[-1] *= 12
            noise = tf.random_normal(noise_shape, stddev=noise_std)
            outputs = tf.concat([outputs, noise], axis=-1)        
...
#                    outputs = tf.layers.conv2d_transpose(outputs, self.depths[i], [5, 5], strides=(2, 2), padding='SAME',
#                                           kernel_initializer=tf.truncated_normal_initializer(stddev=0.02))
#                    if bn and i<len(self.depths)-1:
#                        outputs = tf.nn.relu(tf.layers.batch_normalization(outputs, training=training), name='outputs')
#                    elif i<len(self.depths)-1:
#                        outputs = tf.nn.relu(outputs, name='outputs')
            # output images
...
#                outputs = tf.tanh(outputs, name='sigmoid')
        self.reuse = True
...
#        self.z = tf.random_uniform([self.batch_size, self.z_dim], minval=-1.0, maxval=1.0)
    ...
    def __call__(self, X, noise_std, training = 1.0):
        ...
        training_bool = tf.less(training, 0.5)  
        encoder_output = self.enc(X, training = training_bool)
        
        bits = encoder_output
#        bits = tf.clip_by_value(encoder_output, clip_value_min = -1.,
#                             clip_value_max = 1.) 
#        bits = binary_quantizer(in_q, mode=training)
        
        ## Adding noise to make them independent!!
#        bits = bits + tf.random_normal(tf.shape(bits), stddev=0.1)  
        
        coder_output = self.dec(bits, noise_std, training=training_bool)
        
        print('coder_output', coder_output.get_shape().as_list())
        print('X', X.get_shape().as_list())
