...
gan = GAN(batch_size)
gen_output, d_real, d_fake = gan(X[:, :-1, :], training = training)
...
#    coder_opt =  tf.train.AdamOptimizer(learning_rate, beta1 = 0.5).minimize(coder_cost, var_list=coder_vars)
    ...
    coder_opt =  tf.train.AdamOptimizer(learning_rate).minimize(coder_cost, var_list=coder_vars)
...
#            with tf.device('/gpu:0'):
            ...
            for j in range(coder_update_cycles):
                batch_xs = data_parser(training_data[file_idx], input_dim, batch_size, down_ratio)
                _, coder_cost_adv_= sess.run([coder_opt, coder_cost_adv],
...
#                summary_writer.add_summary(ss, i)  #tensorboard
            print("epoch:", '%02d' % (epoch + 1),
                  ...
                  "coder_cost_w =", "{:.3f}".format( (10**4) * coder_cost_adv_),
