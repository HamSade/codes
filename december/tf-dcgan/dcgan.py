...
training_default = False #corrected
...
class Generator:
    ...
    def __call__(self, inputs, training=training_default):  #corrected
...
class Discriminator:
    ...
    def __call__(self, inputs, training=training_default, name=''): #corrected
...
class DCGAN:
    ...
    def sample_images(self, row=8, col=8, inputs=None):
        if inputs is None:
            inputs = self.z
        images = self.g(inputs, training= True)  #corrected
